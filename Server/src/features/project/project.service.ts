...
export async function estimateFeature(id: string, lvl: number, estimate: string, res: Response) {
  await projectRepository.estimateFeature(id, lvl, estimate).then((result) => {
    res.send(result);
  });
}

export async function getProject(id: string, res: Response) {
  await projectRepository.getProjectById(id).then((result) => {
    console.log(result);
    if (!result) {
